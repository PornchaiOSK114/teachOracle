...
ใช้ AGENTS.md เป็นกติกาหลัก พร้อม docs/SETUP.md และ docs/PROJECT_STATE.md
แทน README/คู่มือที่ขัดกัน ลบ playbook, handoff และแผนเริ่มงานที่หมดอายุ รวมถึง snapshot เก่าในเครื่อง
การแก้ไขรอบนี้เป็นเอกสาร ไม่ได้เปลี่ยน runtime หรือแก้งานค้างของเว็บ
...
## 2026-08-24 — หน้า /lab, /en, /en/lab ที่หนังสือชี้มา

### สรุป

URL สามตัวนี้ถูกพิมพ์ลงในหนังสือไปแล้ว แก้ไม่ได้อีก จึงต้องมีหน้ารองรับให้ครบ
ตรวจจากไฟล์ต้นฉบับจริง (PDF ฉบับไทย + epub ฉบับอังกฤษ) ไม่ได้เดาเอา

| URL | พิมพ์ไว้ที่ไหน | ข้อความในเล่ม |
| :--- | :--- | :--- |
| `/lab` | คำนำฉบับไทย | "ดาวน์โหลดชุดติดตั้งแล็บได้ที่" |
| `/en/lab` | คำนำฉบับอังกฤษ | "Download the lab setup here" |
| `/en` | ฉบับอังกฤษ 3 จุด (หน้าลิขสิทธิ์ · คำนำ · ท้ายเล่ม) | "the current errata list" |

**บทติดตั้งแล็บในเล่มไม่ได้บอกแหล่งดาวน์โหลดไว้ที่อื่นเลย** ตัวชี้ทางเดียวคือ `/lab` ในคำนำ
ถ้าหน้านี้ 404 ผู้อ่านจะหาสคริปต์ไม่เจอและทำแล็บไม่ได้ตั้งแต่บทแรก

### ที่เพิ่ม

| ไฟล์ | ทำอะไร |
| :--- | :--- |
| `app/lab/page.tsx` | หน้าชุดติดตั้งแล็บภาษาไทย |
| `app/en/lab/page.tsx` | หน้าเดียวกันภาษาอังกฤษ |
| `app/en/page.tsx` | หน้าฉบับอังกฤษ + รายการ errata (ตอนนี้ยังว่าง แสดงสถานะตรง ๆ ไม่กุรายการ) |
| `lib/site.ts` | `labKitTh` / `labKitEn` / `labLinks()` / `errataEn` / `author.nameEn` |
| `app/globals.css` | Section 19 — สไตล์บล็อกโค้ดใน `.product-prose` (เดิมไม่มี ทำให้ pre ไม่มีพื้นหลัง) |
| `app/sitemap.ts` | เพิ่มสามหน้านี้ |

### ตัดสินใจอะไรไว้บ้าง

**ไม่ทำ redirect ใช้หน้าจริงแทน** — Cloudflare ของโดเมนนี้ตั้งเป็น DNS only (เมฆเทา)
traffic ไม่ได้วิ่งผ่าน Cloudflare เลย Redirect Rules จึงใช้ไม่ได้ตั้งแต่ต้น
และ URL ที่พิมพ์ในหนังสือแก้ไม่ได้ ถ้าเด้งตรงไป GitHub วันที่ย้าย repo ลิงก์ในเล่มจะตายทันที
หน้าจริงบนโดเมนตัวเองทำให้เปลี่ยนปลายทางทีหลังได้โดยไม่กระทบหนังสือ

**ให้ Google เก็บดัชนีตามปกติ ไม่ใส่ noindex** — เดิมคิดจะซ่อน แต่ repo บน GitHub เป็น public
Google เก็บได้อยู่ดี การซ่อนฝั่งเว็บเราจึงไม่ได้ปิดอะไรจริง แลกกับการเสียช่องทางให้คนเจอหนังสือ
(ฉบับอังกฤษขายบน Amazon การมีหน้าเว็บที่ค้นเจอเป็นข้อได้เปรียบ)

**ปุ่มดาวน์โหลดยังไม่เปิด** — `NEXT_PUBLIC_LAB_REPO_URL` ยังว่างเพราะยังไม่ได้สร้าง repo
หน้าจะแสดงกล่องบอกทางสำรอง (ไฟล์ .zip ที่แนบไปกับอีเมล) แทนปุ่มตายที่กดแล้ว 404
พอสร้าง repo แล้วตั้งตัวแปรตัวเดียว ปุ่มขึ้นเองทั้งสองภาษา

### ที่ต้องระวัง

**หนังสือสั่งให้รัน `@setup_lab.sql` แต่ไฟล์จริงในซิปชื่อ `ch00_setup_lab.sql`**
คนที่พิมพ์ตามเล่มเป๊ะ ๆ จะเจอ `SP2-0310: unable to open file`
แก้โดยเพิ่มไฟล์ `setup_lab.sql` ที่ข้างในมีบรรทัดเดียว `@@ch00_setup_lab.sql` ลงไปในชุด
(ยังไม่ได้ทำ รอเจ้าของเว็บจัดการชุดสคริปต์)

### การตรวจสอบ

- `tsc --noEmit` ผ่าน · `next build` ผ่าน ทั้งสามหน้าเป็น static
- `next start` แล้วเรียกจริง ได้ 200 ทั้ง `/lab`, `/en`, `/en/lab`
- `sitemap.xml` มีสามหน้าใหม่ครบ
- ถ่ายภาพหน้าจอจอคอม 1180px และมือถือ 390px — ไม่มีการล้นขอบจอ (`scrollWidth - clientWidth = 0`)
- ข้อมูลบนหน้าทุกตัว (ข้อกำหนดก่อนรัน · สามค่าที่ต้องแก้ · จำนวนแถวของ 3 ตาราง · คำสั่งรัน)
  ดึงจากเนื้อหาในหนังสือจริง ไม่ได้แต่งเพิ่ม
