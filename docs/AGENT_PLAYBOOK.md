...
### 🔗 กติกา URL (ใช้ร่วมกันทุกเว็บในตระกูลนี้)

| ประเภท | รูปแบบ | ตัวอย่าง |
| :--- | :--- | :--- |
| หน้ารวมบทความ | `/articles` | `teedba.com/articles` |
| บทความ | `/articles/<slug>` | `teedba.com/articles/read-execution-plan` |
| หน้ารวมคอร์ส | `/courses` | |
| คอร์ส (ถ้ามีหน้าแยก) | `/courses/<slug>` | |

**ห้ามใช้ `/blog`** — `kruteekidcode.com` เคยใช้แล้วต้องย้ายมา `/articles` ทีหลัง (ก.ค. 2026)
เหตุผล: คำว่า "articles" สื่อถึงบทความเชิงวิชาการซึ่งตรงกับภาพ expert มากกว่า "blog"
ที่สื่อถึงบันทึกส่วนตัว และเมื่อทุกเว็บใช้เหมือนกันจะสลับไปมาได้โดยไม่สับสน

**ชื่อไฟล์ = slug** ตัดนามสกุลออก ไม่ใส่วันที่หรือ ID คั่น (`/2026/07/slug` ❌)
URL จะได้อ่านออกและไม่เปลี่ยนตามเวลา

> ⚠️ **ถ้าจำเป็นต้องเปลี่ยน URL ของเว็บที่ขึ้นแล้ว** ต้องใส่ redirect ถาวรใน `next.config.ts`
> เสมอ และ**ห้ามลบทิ้งภายหลัง** — ลิงก์เก่าที่คนแชร์ไว้อยู่ในอินเทอร์เน็ตตลอดไป
>
> ```ts
> async redirects() {
>   return [
